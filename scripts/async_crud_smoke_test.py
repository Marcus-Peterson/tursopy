#!/usr/bin/env python
"""
Async CRUD smoke test against Turso using AsyncTursoConnection.
...
async def run() -> int:
    ...
    table = f"crud_async_demo_{suffix}"
    ...
    async with AsyncTursoConnection() as conn:
        try:
            ...
            # Update row
            await conn.execute_query(
                f"UPDATE {table} SET age = ? WHERE id = ?", [23, row_id]
            )
            print("UPDATE:ok")

            # Read after update
            res2 = await conn.execute_query(
            ...
            print("CRUD_ASYNC_SMOKE:success")
            return 0
        except Exception as e:
            print("CRUD_ASYNC_SMOKE:error:", e)
            return 1


if __name__ == "__main__":
    raise SystemExit(anyio.run(run))
